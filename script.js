...
function renderSeasonEnd(area) {
  ...
  // Resolve champions ONCE and cache in state to avoid random re-rolls on every render
  if (!state._seasonChampSAM) state._seasonChampSAM = resolveChamp("sam");
  if (!state._seasonChampSAF) state._seasonChampSAF = resolveChamp("saf");
...
function startNextSeason() {
  ...
  // Use cached champions from renderSeasonEnd (avoids re-rolling tie-break)
  const champSAM = state._seasonChampSAM || resolveChamp("sam");
  const champSAF = state._seasonChampSAF || resolveChamp("saf");
