...
function checkAllDone() {
  ...
  // Calculate champions ONCE here, lock in state before any rendering
  state._seasonChampSAM = resolveChamp("sam");
  state._seasonChampSAF = resolveChamp("saf");
...
function renderSeasonEnd(area) {
  ...
  // Champions already calculated and locked in checkAllDone() - just read them
  const champSAM = state._seasonChampSAM || "N/D";
  const champSAF = state._seasonChampSAF || "N/D";
  ...
  // NOTE: titles are recorded only once in startNextSeason(), NOT here
}
...
function resolveChamp(comp) {
  ...
  const topPts = state.leagues[comp].table[sorted[0]].pts;
  // Collect ALL wrestlers tied at the top
  const tied = sorted.filter(w => state.leagues[comp].table[w].pts === topPts);
  if (tied.length === 1) return tied[0];
  // Multiple tied — spin wheel among ALL of them
  return spinWheelTie(tied);
}
...
function recordSeasonTitles(champM, champF) {
  // Guard: only record once per season
  ...
  const titleM = `World Heavyweight S${state.season}`;
  const titleF = `World IC Woman S${state.season}`;
  if (!state.megaStats[champM].titles.includes(titleM))
    state.megaStats[champM].titles.push(titleM);
  if (!state.megaStats[champF].titles.includes(titleF))
    state.megaStats[champF].titles.push(titleF);
}
...
function startNextSeason() {
  ...
  // Champions were locked in checkAllDone() - never re-roll
  const champSAM = state._seasonChampSAM;
  const champSAF = state._seasonChampSAF;
  if (!champSAM || !champSAF) { toast("Errore: campioni non calcolati!"); return; }

  // Record league titles ONCE here
  recordSeasonTitles(champSAM, champSAF);

  // Record cups/champions titles with guard
  const coppaMWinner = state.cups.male?.winner;
  const coppaFWinner = state.cups.female?.winner;
  const champWinner = state.champions?.winner;
  if (coppaMWinner) {
    if (!state.megaStats[coppaMWinner]) state.megaStats[coppaMWinner] = { pts: 0, wins: 0, losses: 0, titles: [] };
    const t = `United States S${state.season}`;
    if (!state.megaStats[coppaMWinner].titles.includes(t)) state.megaStats[coppaMWinner].titles.push(t);
  }
  if (coppaFWinner) {
    if (!state.megaStats[coppaFWinner]) state.megaStats[coppaFWinner] = { pts: 0, wins: 0, losses: 0, titles: [] };
    const t = `Divas S${state.season}`;
    if (!state.megaStats[coppaFWinner].titles.includes(t)) state.megaStats[coppaFWinner].titles.push(t);
  }
  if (champWinner) {
    if (!state.megaStats[champWinner]) state.megaStats[champWinner] = { pts: 0, wins: 0, losses: 0, titles: [] };
    const t = `Champions S${state.season}`;
    if (!state.megaStats[champWinner].titles.includes(t)) state.megaStats[champWinner].titles.push(t);
  }
  ...
  state.history.push({
    ...
    serieAM: champSAM,
    serieAF: champSAF,
    champions: champWinner || null,
    coppaNM: coppaMWinner || null,
    coppaNF: coppaFWinner || null,
  ...
  // Clear cached season champions
  delete state._seasonChampSAM;
  delete state._seasonChampSAF;
